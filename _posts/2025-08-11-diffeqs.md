...
In diffusion models, we \textit{design} a very simple, first-order SDE (specifically, this is an Ito SDE) which transforms our data $x \in \mathbb{R}^{d}$ in a stochastic way. We choose this \textit{forward SDE} to look like this: 
...
This gives us a way of describing the dynamics of $x$ (equivalent mathematically to a \textit{particle} in $\mathbb{R}^{d}$)This assumes that $x$ is just some point in $\mathbb{R}^{d}$. The whole point of generative modeling, though, is that $x$ doesn't exist in a vacuum. Rather, it is a sample from a data \textit{distribution}, $x \sim p(x)$, where $p(x)$ is the data's probability density. This introduces another probabilistic component - but strangely enough, brings us back to PDE language.

## PDEs

We've imposed some stochastic forward dynamics of our particle $x$ over time. However, if we think of evolving \textit{all} of the particles over our data density $p(x)$ in time, a PDE naturally falls out. Let's see why.

It intuitively makes sense that when we evolve each particle $x \sim p(x)$ in time according to the above SDE, we will obtain a \textit{new} distribution, $p_{t}(x)$, at each time $t$. Whenever we apply \textit{any} transformation to a \textit{probability distribution}, however, we have to remember a crucial ground rule: probability distributions always integrate to 1. Otherwise, it wouldn't be a proper probability distribution! This means that if we think of each particle in $p_{t}(x)$ as some tiny amount of mass, all that the SDE should be allowed to do is move all of this (probability) mass around - not create or destroy it. This condition is a conservation law, very loosely given by 
...
s(x,t) = s_{t}(x)= \nabla_{x}\log(p_{t}(x)). 
$$
If you're already familiar SBDMs, you know that this score $s_{t}$ is particularly crucial in the \textit{reverse} SDE, as this is how one ultimately samples from the complex distribution. The score also certainly appears in the forward SDE and associated FP-PDE (as we will show below), and its role becomes even more apparent as as we move into PF-ODE territory. 
...
$$
we observe (hopefully very little astuteness required!) that our rearrangement follows a continuity equation form with a \textit{velocity field} of the form 
$$
