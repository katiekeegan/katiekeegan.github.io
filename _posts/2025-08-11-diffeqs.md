...
s(x,t) = s_{t}(x)= \nabla_{x}\log(p_{t}(x)). 
$$
If you're already familiar with SBDMs, you know that this score $s_{t}$ is particularly crucial in the \textit{reverse} SDE, as this is how one ultimately samples from the complex distribution. The score also certainly appears in the forward SDE and associated FP-PDE (as we will show below), and its role becomes even more apparent as as we move into PF-ODE territory. 
